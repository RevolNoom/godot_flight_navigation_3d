...
static func create_new(tree_depth: int = 2, act1nodes: PackedInt64Array = [], subgrid_states: PackedInt64Array = []) -> SVO_V3:
	var svo = SVO_V3.new()
	...
	if it.is_subgrid_voxel():
		...
		for neighbor in [
			...
			if Morton3.ge(neighbor[1], 0) and Morton3.le(neighbor[1], 63):
				...
				if neighbor_it.svolink == SVOLink.NULL:
					continue
...
func _smallest_voxels_on_surface(face: SVOIterator.DataField, svolink: int) -> PackedInt64Array:
	...
	var node = SVOIteratorRandom._new(self, svolink)
	
	if layer == 0:
		# This node is all free space. No need to travel its subgrid
		if node.rubik == SVONode.Subgrid.EMPTY:
			return [svolink]
