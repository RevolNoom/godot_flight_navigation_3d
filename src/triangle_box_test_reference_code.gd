## Deprecated. Fast triangle-box test as described by Michael Schwarz and Hans-Peter Seidel.
##
## This is the first version of the Triangle-Box test. It stores and uses Vector2 
## and Vector3 (which use float32) for calculation. The accuracy loss was terrible
## when the triangles lie on FlightNavigation3D bounding box surface, result in 
## those triangles not voxelized.
##
## Due to being easier to read and understand than the currently used Triangle-
## Box Test version, this file is kept for reference.
extends RefCounted
class_name TriangleBoxTest_ReferenceCode
...
func _init(v: PackedVector3Array, dp: Vector3):
	...
	for i in range(0, 3):
		_ne_xy[i] = Vector2(-e[i].y, e[i].x) if n.z >= 0.0 else Vector2(e[i].y, -e[i].x)
		_ne_yz[i] = Vector2(-e[i].z, e[i].y) if n.x >= 0.0 else Vector2(e[i].z, -e[i].y)
		_ne_zx[i] = Vector2(-e[i].x, e[i].z) if n.y >= 0.0 else Vector2(e[i].x, -e[i].z)
...
func _plane_overlaps(p: Vector3) -> bool:
	...
	#return Vector2(np + _d1, 0).dot(Vector2(np + _d2, 0)) <= 0
...
func _projection_2d_overlaps(p: Vector3) -> bool:
	...
	#return Vector3(
		#(_ne_xy[0].dot(p_xy) + _de_xy[0]),
		#(_ne_xy[1].dot(p_xy) + _de_xy[1]),
		#(_ne_xy[2].dot(p_xy) + _de_xy[2]))\
		#>= Vector3.ZERO\
		#and\
		#Vector3(
		#(_ne_yz[0].dot(p_yz) + _de_yz[0]),
		#(_ne_yz[1].dot(p_yz) + _de_yz[1]),
		#(_ne_yz[2].dot(p_yz) + _de_yz[2]))\
		#>= Vector3.ZERO\
		#and\
		#Vector3(
		#(_ne_zx[0].dot(p_zx) + _de_xz[0]),\
		#(_ne_zx[1].dot(p_zx) + _de_xz[1]),\
		#(_ne_zx[2].dot(p_zx) + _de_xz[2]))\
		#>= Vector3.ZERO
...
static func _auto_test_triangle_extended(v: PackedVector3Array, box_range: PackedInt64Array, solid_voxels: PackedVector3Array, box_size:= Vector3(1,1,1)):
	...
	for x in box_range:
		for y in box_range:
			for z in box_range:
				var key = Vector3(x, y, z) * box_size
				test_result[key] = test.overlap_voxel(key)
	...
	for key in test_result.keys():
		if test_result[key] != expected_result[key]:
			errors += 1
			print("Got %s, expected %s for %s" % [test_result[key], expected_result[key], key])
