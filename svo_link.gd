...
static func to_navspace(
	...
	var node_size = extent / (2 ** (svo._nodes.size() - link_layer - 1))
	return (Morton3.decode_vec3(node.morton) + Vector3.ONE*0.5) * node_size


## @gposition: A global position inside navspace
## @extent: The length of one side of the navigation space (assumed to be cube)
##
...
static func from_navspace(
		navspace: NavigationSpace3D, 
		gposition: Vector3) -> int:
	var position = navspace.to_local(gposition) - navspace._extent_origin
	...
	return SVOLink.from(link_layer, link_offset, Morton3.encode64v(subgridv))
